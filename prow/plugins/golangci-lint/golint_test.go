...
func Test_problemsInFiles(t *testing.T) {
	type args struct {
		r     git.RepoClient
	...
	tests := []struct {
		name  string
		args  args
		want  map[string]map[int]data.Issue
		want1 []github.DraftReviewComment
	}{
		{
			name: "test",
			args: args{
				r: nil,
				files: map[string]string{
					"caas/farmapi/cmd/farmctl/cmd/region.go": "",
				},
			},
			want:  nil,
			want1: nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1 := problemsInFiles(tt.args.r, tt.args.files)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("problemsInFiles() got = %v, want %v", got, tt.want)
			}
			if !reflect.DeepEqual(got1, tt.want1) {
				t.Errorf("problemsInFiles() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
