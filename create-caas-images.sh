...
export REPO=registry.app.corpintra.net/caas-ci
export TAG="v$(date +"%Y%m%d")-$(git rev-parse --short HEAD)"

make -C prow build-images

# core Prow images
for COMPONENT in crier deck hook horologium prow-controller-manager sinker status-reconciler tide exporter
do
...
# decoration images
for COMPONENT in clonerefs initupload entrypoint sidecar
do
    docker tag ko.local/${COMPONENT}:${TAG} ${REPO}/${COMPONENT}:${TAG}
    docker push ${REPO}/${COMPONENT}:${TAG}
done
...
# external plugins
for COMPONENT in lenses mention cherrypicker
do
    docker tag ko.local/${COMPONENT}:${TAG} ${REPO}/${COMPONENT}:${TAG}
    docker push ${REPO}/${COMPONENT}:${TAG}
done
